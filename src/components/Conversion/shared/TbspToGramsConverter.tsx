...
import { tbspToGrams } from "@/components/Conversion/shared/conversionUtils";
import { formatNum } from "@/components/Maths/shared/mathUtils";
import { CalculatorEmptyResult } from "@/components/shared/calculatorFields";

type Props = { emptyHint: string };

export function TbspToGramsConverter({ emptyHint }: Props) {
  const resultRef = useRef<HTMLElement>(null);
  const [tbsp, setTbsp] = useState("2");
  ...
  const run = () => {
    setError(null);
    const t = Number(tbsp.trim());
    const d = Number(density.trim());
    if (!Number.isFinite(t) || t < 0) {
      setError("Enter a valid number of tablespoons.");
      setResult(null);
      return;
    }
  ...
  const form = (
    <div className="flex flex-1 flex-col gap-3 sm:gap-4 md:gap-6">
      <div>
        <label htmlFor="tbsp-to-g-tbsp" className="mb-1.5 block text-[13px] font-medium text-[#334155] sm:text-[15px]">
          Tablespoons (tbsp)
        </label>
        <input
          id="tbsp-to-g-tbsp"
      ...
      <div>
        <label htmlFor="tbsp-to-g-density" className="mb-1.5 block text-[13px] font-medium text-[#334155] sm:text-[15px]">
          Density (g/ml)
        </label>
        <input
          id="tbsp-to-g-density"
          type="number"
          min={0.001}
          step="any"
          value={density}
        ...
        />
        <p className="mt-1 text-[11px] text-[#94a3b8] sm:text-[12px]">US tbsp (14.79 ml). Water ≈ 1 g/ml.</p>
      </div>
    ...
    <CalculatorTwoPanel
      ...
      disclaimer={
        <p className="mt-4 text-center text-[11px] italic leading-snug text-[#9ca3af] sm:mt-8 sm:text-[13px]">
          Mass depends on ingredient density. Default is water-like (1 g/ml).
        </p>
      }
    />
  );
