...
import { gramsToMl } from "@/components/Conversion/shared/conversionUtils";

type Props = {
  emptyHint: string;
};

export function GramsToMlConverter({ emptyHint }: Props) {
    ...
    <div className="flex flex-1 flex-col gap-3 sm:gap-4 md:gap-6">
      ...
      <div>
        ...
        <p className="mt-1 text-[11px] text-[#94a3b8] sm:text-[12px]">
          Water ≈ 1 g/ml. Milk ≈ 1.03, oil ≈ 0.92, honey ≈ 1.42.
        </p>
      </div>
    ...
    <CalculatorTwoPanel
      ...
      disclaimer={
        <p className="mt-4 text-center text-[11px] italic leading-snug text-[#9ca3af] sm:mt-8 sm:text-[13px] sm:leading-normal">
          Mass to volume depends on density. Default assumes water.
        </p>
      }
    />
  );
